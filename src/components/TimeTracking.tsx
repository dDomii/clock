...
export function TimeTracking() {
  ...
  const fetchHoursProgress = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/user-hours-progress', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const updateRequiredHours = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/user-required-hours', {
        method: 'PUT',
        headers: {
  ...
  const fetchOvertimeNotifications = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/overtime-notifications', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      if (data.length > 0) {
  ...
  const resetAndClockIn = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/reset-clock-in', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const submitOvertimeRequest = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/overtime-request', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
                    ...
                    <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 rounded-2xl p-6 border border-blue-700/30 backdrop-blur-sm h-full">
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg border border-slate-600/30">
                          ...
                          <span className="font-semibold text-blue-400">{Number(hoursProgress.requiredHours).toFixed(1)}h</span>
                        </div>
                        
                        <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg border border-slate-600/30">
                          <span className="text-slate-400 text-sm">Worked Hours:</span>
                          <span className="font-semibold text-emerald-400">{Number(hoursProgress.workedHours).toFixed(1)}h</span>
                        </div>
                        
                        <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg border border-slate-600/30">
                          <span className="text-slate-400 text-sm">Remaining:</span>
                          <span className={`font-semibold ${hoursProgress.isCompleted ? 'text-emerald-400' : 'text-orange-400'}`}>
                            {hoursProgress.isCompleted ? 'Completed!' : `${Number(hoursProgress.remainingHours).toFixed(1)}h left`}
                          </span>
                        </div>
